...
public class UserSkillController {
    ...
    public ResponseEntity<UserSkill> createSkill(@PathVariable String uuid, @RequestBody UserSkill skill) {
        ...
        skill.setUser(user);
        return ResponseEntity.ok().body(userSkillServiceImpl.createSkill(skill));
    }

    @GetMapping
    public ResponseEntity<List<UserSkill>> getAllUserSkills() {
        return ResponseEntity.ok().body(userSkillServiceImpl.getAllUserSkills());
    }
    ...
    public ResponseEntity<Optional<List<UserSkill>>> getAllSkillsForUser(@PathVariable String uuid) {
        ...
        return ResponseEntity.ok().body(userSkillServiceImpl.getUserSkillsByUser(user));
    }

    @PatchMapping("{id}")
    public ResponseEntity<UserSkill> updateSkillDetails(@PathVariable String id, @RequestBody Map<String, Object> skill)
            throws JsonMappingException {
        UserSkill existing = userSkillServiceImpl.getSkillById(Long.parseLong(id)).get();

        var newDetails = objectMapper.updateValue(existing, skill);
        try {
            userSkillServiceImpl.updateSkill(newDetails);

        } catch (Exception e) {
            ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Failed to Update the selected skill because of: " + e.getMessage());
        }
        return ResponseEntity.ok().body(userSkillServiceImpl.getSkillById(Long.parseLong(id)).get());
    }

    @DeleteMapping("{id}")
    public ResponseEntity<?> deleteUserSkill(@PathVariable String id) {
        try {
            userSkillServiceImpl.deleteUserSkillById(Long.valueOf(id));
        } catch (Exception e) {
            ResponseEntity.status(HttpStatus.BAD_REQUEST)
